...
function normalizeCountryDisplay(value = "") {
  ...
  const code = cleanedValue.toUpperCase().replace(/[^A-Z]/g, "");

  if (countryNames[code]) return countryNames[code];
  if (cleanedValue.length > 3) return titleCase(cleanedValue);
  return cleanedValue;
}
...
function normalizePassportDetails(passportDetails = {}) {
  ...
  return {
    fullName,
    sex,
    nationality: normalizeCountryDisplay(passportDetails.nationality),
    issuingCountry: normalizeCountryDisplay(passportDetails.issuingCountry),
